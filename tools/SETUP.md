...
`headcount.json` 을 메모장으로 열어 숫자만 고치고 저장하면 됩니다.
...
10분마다 도는 `CRM4 화면갱신` 이 바뀐 것을 보고 화면을 다시 만들어 올립니다.
그래서 **화면을 보는 사람 모두가 같은 숫자를 봅니다.**

화면의 시흥·천안 카드에서 인원을 눌러 바꿔 볼 수도 있는데, 그것은 그 사람 화면에서만
바뀌고 새로 고치면 돌아옵니다. 바꿔 본 동안에는 표 위에 그렇다고 표시가 뜹니다.
모두에게 반영하려면 이 파일을 고쳐야 합니다.
